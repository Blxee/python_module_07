...
class Deck:
    """Deck of cards which manages various card operations."""
    ...
    def add_card(self, card: Card) -> None:
        ...
        if not isinstance(card, Card):
            print(
                "[Error]: invalid argument type for add_card().", file=stderr
            )
            exit(1)
    ...
    def remove_card(self, card_name: str) -> bool:
        ...
        if not isinstance(card_name, str):
            print(
                "[Error]: invalid argument type for remove_card().",
                file=stderr,
            )
            exit(1)
    ...
    def get_deck_stats(self) -> Dict:
        ...
        for card in self.cards:
            ...
            card_type: str = f"{card.type.value.lower()}s"
